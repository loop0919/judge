...
func TestRankPenalizesOnlyWrongSubmissionsBeforeFirstAC(t *testing.T) {
	...
	if zero := Rank(attempts, start, 0); zero[1].TimeMS != 10*60000 {
	...
	if r := Rank(pending, start, 5)[0]; r.TimeMS != 15*60000 || r.Problems["A"].Pending != 1 {
		t.Fatal(r)
	}
	pending[0].Verdict = "AC"
	if r := Rank(pending, start, 5)[0]; r.TimeMS != 60000 || r.Problems["A"].Wrong != 0 {
		t.Fatal(r)
	}
	...
	if r := Rank(multi, start, 7)[0]; r.Points != 300 || r.TimeMS != 44*60000 {
		t.Fatal(r)
	}
}
