...
<template>
  <article class="blog-article">
    ...
    <section id="output">
      ...
      <p>出力生成は登録済みの全テストケースを対象にします。
開始位置や件数の指定は不要です。
実行前の確認画面に、期待出力を上書きする件数が表示されます。</p>
    </section>
    <section id="validation">
      <h2>入力が制約を満たすか検証する</h2>
      <p>種類を「入力検証」にすると、登録済みの全テストケースの入力が、それぞれ標準入力へ渡されます。終了コード0で合格、0以外の終了コードや異常終了で不合格になります。標準出力は保存せず、入力と期待出力も変更しません。</p>
      <p>次のC++17の例では、0以上10億以下の整数が2つだけあることを検証します。末尾の空白と改行は許可します。</p>
      <pre><code>{{ validationCode }}</code></pre>
      <p>言語とコードを設定し、「検証する」を押してください。
範囲を指定せず全テストケースを検証し、ケースごとの結果が表示されます。
時間超過（TLE）、メモリ超過（MLE）、出力超過（OLE）は「検証未完了」です。コードや実行条件を確認して再実行してください。</p>
    ...
    <section id="steps">
      ...
      <ol>
        ...
        <li>「生成と検証」に戻り、「出力生成」で言語と解答コードを設定します。</li>
        <li>「生成する」を押し、期待出力を上書きする確認に同意します。完了後に入出力の組を確認してください。</li>
      </ol>
      <p>入力生成・出力生成・入力検証のコードと言語はそれぞれ下書きに保存されます。公開済み・コンテスト登録済みの問題では、実行前の変更と生成結果を「保存」で保存してください。それ以外の下書きは自動保存されます。コンテストの採点内容は保存時に更新されます。単独の問題の採点への反映には問題一覧の「投稿」から公開するか、「問題管理」から公開内容を更新してください。</p>
    </section>
    <section id="limits">
      <h2>上限と失敗時の動作</h2>
      <ul>
      ...
      <p>コンパイルエラー、実行エラー、時間超過、容量超過、ファイル検証の失敗がある場合は、結果を反映せず既存のテストケースを保持します。画面のエラーを確認し、コードやテストケースを修正して再実行してください。</p>
    </section>
    <section id="check">
      <h2>生成した答えも確認する</h2>
