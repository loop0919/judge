...
<template>
  <section class="submission-detail">
    ...
    <header class="submission-header"><h1>{{ item?.easyTest ? 'Easy Test の結果' : '提出結果' }}</h1><p v-if="item" class="submission-id">ID: {{ item.id }}</p></header>
    ...
    <template v-if="item">
      ...
      <section v-if="item.result?.checkerLog" class="compile-log" aria-labelledby="checker-log-title"><h2 id="checker-log-title">ジャッジコードの診断（作問者のみ）</h2><pre>{{ item.result.checkerLog }}</pre></section>
