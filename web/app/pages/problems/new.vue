...
<template>
  <div class="author-page">
    ...
    <div v-if="section === 'statement'" class="author-fields">
      ...
      <div class="field"><label for="problem-difficulty">難易度（作成者設定）</label><select id="problem-difficulty" v-model="draft.difficulty" :disabled="!ready || publishing"><option :value="null">未設定</option><option v-for="(grade, index) in difficultyGrades" :key="grade" :value="index + 1">Lv.{{ index + 1 }} · {{ grade }}</option></select></div>
      <div>
