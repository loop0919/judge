...
NuxtはアクセストークンをHttpOnly、SameSite=LaxのCookieに保存し、ブラウザーへトークンをJSONで返さない。
...
ログアウトはCookieを削除する。トークンの失効APIや自動更新は実装していないため、有効期限後は再ログインする。
...
今回の変更にはAWSへのDB作成やデプロイ、提出と採点の接続は含まれない。
