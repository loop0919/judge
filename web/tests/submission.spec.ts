...
test('language field searches inline and supports keyboard selection and cancellation', async ({ page }) => {
  ...
  await expect(language).toHaveValue('C++17 (GCC)')
  await language.click()
  await expect(options).toHaveText(['-- 未選択 --', 'C++17 (GCC)', 'Python 3.14'])
  for (const query of ['Python', 'pYtN', ' Ｐｙ Ｔｎ ']) {
    await language.fill(query)
    await expect(options).toHaveText(['Python 3.14'])
  }
  ...
  await language.fill('pytn')
  await language.press('Enter')
  await expect(language).toHaveValue('Python 3.14')
  await expect(language).toHaveAttribute('aria-expanded', 'false')
  ...
  await expect(language).toHaveValue('Python 3.14')
  await expect(language).toHaveAttribute('aria-expanded', 'false')
  await language.fill('Ｃ＋＋')
  await expect(options).toHaveText(['C++17 (GCC)'])
  await page.getByRole('heading', { name: '提出', exact: true }).click()
  await expect(language).toHaveValue('Python 3.14')
  await page.reload()
  await expect(language).toHaveValue('Python 3.14')
  await page.getByRole('button', { name: '言語の候補を開く' }).click()
  await expect(options).toHaveCount(3)
  await language.press('ArrowUp')
  const activeId = await language.getAttribute('aria-activedescendant')
  await expect(page.locator(`[id="${activeId}"]`)).toHaveText('C++17 (GCC)')
  await language.press('Enter')
  await expect(language).toHaveValue('C++17 (GCC)')
...
test('language selection survives reloads and returning to the problem', async ({ page }) => {
  ...
  await expect(language).toHaveValue('C++17 (GCC)')
  await language.click()
  await page.getByRole('option', { name: 'Python 3.14', exact: true }).click()
  await page.reload()
  await expect(language).toHaveValue('Python 3.14')
  ...
  await expect(language).toHaveValue('Python 3.14')
  await language.click()
  await page.getByRole('option', { name: 'C++17 (GCC)', exact: true }).click()
  await page.reload()
  await expect(language).toHaveValue('C++17 (GCC)')
})
...
test('unselected language disables both actions and survives reloads', async ({ page }) => {
  ...
  await language.click()
  await page.getByRole('option', { name: '-- 未選択 --', exact: true }).click()
  ...
  await page.getByRole('option', { name: '-- 未選択 --', exact: true }).click()
  await page.reload()
  await expect(language).toHaveValue('-- 未選択 --')
  await page.getByLabel('ソースコード', { exact: true }).fill('int main(){}')
  await expect(submit).toBeDisabled()
  await expect(sample).toBeDisabled()
})
...
test('an unavailable saved language falls back to the default', async ({ page }) => {
  ...
  await page.goto(`/problems/${problemId}`)
  await expect(page.getByRole('combobox', { name: '言語' })).toHaveValue('C++17 (GCC)')
})
