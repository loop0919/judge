...
test('test cases can be edited, restored, published and used for submission without public disclosure', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('heading', { name: 'テストケース', exact: true })).toBeVisible()
  await page.getByRole('button', { name: 'テストケースを追加' }).click()
  await page.getByLabel('入力 1', { exact: true }).fill('3 5\n')
  await page.getByLabel('期待出力 1', { exact: true }).fill('8\n')
  await page.getByRole('button', { name: 'テストケースを追加' }).click()
  await page.getByLabel('入力 2', { exact: true }).fill('1000000000 1000000000\n')
  await page.getByLabel('期待出力 2', { exact: true }).fill('2000000000\n')
  await page.getByRole('button', { name: '保存', exact: true }).click()
  await expect(page.getByRole('status')).toHaveText('保存済み')
  const id = new URL(page.url()).searchParams.get('problem')!
  await page.reload()
  await page.getByRole('button', { name: 'テストケース', exact: true }).click()
  await expect(page.getByLabel('期待出力 2', { exact: true })).toHaveValue('2000000000\n')
  ...
  await expect(page.getByLabel('期待出力 2', { exact: true })).toHaveCount(0)
  await expect(page.getByRole('status')).toHaveText('保存済み')
  const privateResponse = await page.request.get(`/api/my/problems/${id}`)
  expect((await privateResponse.json()).draft.testCases).toEqual([{ input: '3 5\n', output: '8\n' }])
  await page.getByRole('button', { name: '問題管理', exact: true }).click()
