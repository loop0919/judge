...
test('create, reorder and edit an unpublished contest; guests cannot inspect its problems', async ({ page, browser }, testInfo) => {
  ...
  await expect(page.getByText('提出一覧と提出コードはコンテスト終了後に公開されます。')).toBeVisible()
...
test('scheduled problems open for submission, then publish with editorial and source after the deadline', async ({ page, browser }, testInfo) => {
  ...
  await expect(bob.getByRole('navigation', { name: '問題メニュー' }).getByRole('link')).toHaveText(['問題', '自分の提出'])
  ...
  await expect(reader.getByRole('navigation', { name: '問題メニュー' }).getByRole('link')).toHaveText(['問題', '自分の提出'])
  ...
  await expect(reader.getByRole('navigation', { name: '問題メニュー' }).getByRole('link')).toHaveText(['問題', '解説', '自分の提出'])
  for (const width of [320, 375, 414, 768]) {
  ...
  await reader.getByRole('link', { name: '時間で公開する問題', exact: true }).click()
