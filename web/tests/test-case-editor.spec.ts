...
test('line numbers track scrolling without rendering every line and names are unique', async ({ page }) => {
  ...
  expect(await gutter.count()).toBeLessThanOrEqual(Math.ceil(await input.evaluate(element => element.clientHeight) / 26) + 2)
  ...
  await input.evaluate(element => { element.scrollTop = 2600; element.dispatchEvent(new Event('scroll')) })
