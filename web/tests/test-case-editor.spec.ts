...
test('difficulty picker shows grades and supports keyboard selection and persistence', async ({ page }, testInfo) => {
  await page.goto('/problems/new')
  const picker = page.getByRole('combobox', { name: '難易度（作成者設定）' })
  await picker.click()
  await expect(page.getByRole('option')).toHaveCount(11)
  await expect(page.getByRole('listbox').locator('.difficulty-dot')).toHaveCount(9)
  await expect(page.getByRole('option', { name: 'Lv.10', exact: true }).locator('svg')).toBeVisible()
  await page.getByRole('option', { name: 'Lv.5', exact: true }).click()
  await expect(picker.locator('.difficulty-dot')).toHaveCSS('background-color', 'rgb(0, 184, 212)')
  await picker.press('ArrowDown')
  await picker.press('End')
  await picker.press('Enter')
  await expect(picker).toContainText('Lv.10')
  await expect(picker.locator('svg')).toBeVisible()
  await picker.press('ArrowDown')
  ...
  for (const width of [1280, 375]) {
    ...
    await page.getByLabel('問題のタイトル', { exact: true }).click()
    await expect(page.getByRole('listbox')).toHaveCount(0)
  }
  await picker.click()
  await picker.press('Home')
  await picker.press('Enter')
  await expect(picker).toContainText('未設定')
  await picker.click()
...
test('append samples preserves the statement, excludes private cases, and saves literal data', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'テストケース', exact: true }).click()
  const append = page.getByRole('button', { name: 'サンプルを問題文に追加', exact: true })
  await expect(append).toBeDisabled()
  await page.getByRole('button', { name: 'テストケースを追加' }).click()
  await page.getByLabel('入力', { exact: true }).fill('private input')
  await expect(append).toBeDisabled()
  await page.getByRole('button', { name: 'テストケースを追加' }).click()
  await page.getByRole('checkbox', { name: 'サンプルケースにする' }).check()
  await page.getByLabel('入力', { exact: true }).fill('3 5\n```\n$literal$')
  await page.getByLabel('出力', { exact: true }).fill('8')
  await page.getByRole('button', { name: 'テストケースを追加' }).click()
  await page.getByRole('checkbox', { name: 'サンプルケースにする' }).check()
