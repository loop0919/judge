...
[ADR 0009](../adr/0009-support-interactive-judge.md)に従い、isolateワーカーで対話用ジャッジを実行する。
2026年9月13日にLightsailで同時実行と資源上限を検証し、提出と同じ8言語で公開した。
開発用Dockerワーカーは対話形式の提出を受け付けない。
...
たとえば、テスト入力に整数Nを保存し、提出へNを伝えて2倍の値を答えさせる対話用ジャッジは次のように書ける。
期待出力は空でよい。
...
with open(sys.argv[1]) as file:
    n = int(file.read())
print(n, flush=True)
assert int(input()) == 2 * n
assert not sys.stdin.read().strip()  # 提出の終了を待ち、余分な出力も検証する
```

対応する提出プログラムは、応答をflushしてから終了する。
...
個別の応答待ちタイマーは設けず、相互待機も対話全体の経過時間で打ち切る。

ワーカー全体の1,536 MiB、swapなし、作業用tmpfs全体512 MiBは維持する。
...
対話形式の実行時は各32タスクとし、コンパイル時の64を維持する。
...
両側の正常終了がそろわず対話全体の経過時間を超えた場合はTLEとし、対話の判定を作問者の診断に残す。
この経過時間には、相互待機と片側の終了後の待機も含まれる。
...
対話用のJavaはヒープ上限を128 MiBとし、VM全体を256 MiBで制限する。
Javaの公開保留は継続し、将来の公開時にもVM全体の消費量を検証する。
...
診断は既存の`checkerLog`に格納し、スペシャルジャッジと同じ認可を適用する。
問題の公開APIはコードを返さず、`interactive: true`だけを返す。
isolateにはbox ID 0と1、専用UID/GID 60000と60001を予約する。
`interactive.py`もruntime digestの計算対象に含める。

DBマイグレーションは不要である。
配置では[ランタイムの再構築と公開](runtime-rollout.md)に従い、既存ジョブの処理後にワーカーとAPIを更新する。
`smoke.py`は既存の通常判定とスペシャルジャッジを検証した後、`interactive_smoke.py`で全対象言語の対話と資源制限を検証する。
対話の検証が失敗した場合も公開用の成功レポートを出さない。
実機検証後に新しいdigestをAPIとbridgeに設定し、既存の公開言語を再開する。

全言語の実機smoke、各ページへの実書き込みを伴うメモリ負荷、公開APIからの7件の提出が成功した。
検証と公開設定の記録は[配置記録](runtime-rollout.md#2026年9月13日のインタラクティブジャッジ対応)に記載する。
